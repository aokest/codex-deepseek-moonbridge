...
>
> **懒人通道**：不想看文档？直接跑 `bash setup.sh`，脚本会自动完成全部安装和配置。遇到问题再看本文档。**

> **只想生成配置文件？** 跑 `python3 generate_config.py -o ~/moon-bridge/`，交互式问答自动生成 config.yml。
...
> **快速生成配置**：使用交互式配置生成器，无需手动编写 YAML：
> ```bash
> python3 generate_config.py -o ~/moon-bridge/
> ```
> 脚本会依次询问各 Provider 的 API Key，跳过不填即不启用该 Provider。
...
| 小米 MiMo | OpenAI 兼容 | `model-name` | `mimo-v2.5-pro` |
...
| Provider | base_url | 协议 | api_version |
|----------|---------|------|-------------|
| DeepSeek 自有 Key | `https://api.deepseek.com/anthropic` | Anthropic 兼容 | 无 |
| 九天 | `https://jiutian.10086.cn/largemodel/moma/api` | OpenAI Chat Completions | `v3` |
| 小米 MiMo | `https://token-plan-sgp.xiaomimimo.com` | OpenAI Chat Completions | `v1`（默认，无需声明） |
...
| Route Slug | 上游模型 ID | 说明 |
|-----------|------------|------|
| `xm-omni` | mimo-v2-omni | 多模态视觉模型，256K 上下文 |
| `xm-v2.5-pro` | mimo-v2.5-pro | 旗舰推理模型，1M 上下文 |
| `xm-v2.5` | mimo-v2.5 | 标准推理模型，1M 上下文 |
| `xm-v2-pro` | mimo-v2-pro | 推理模型，1M 上下文 |

> **注意**：
> - 所有 MiMo 模型均为推理模型（reasoning），需要足够 `max_tokens`（建议 200+），否则可能返回空内容。
> - `mimo-v2-omni` 是唯一支持多模态/视觉的模型。
> - `mimo-v2-flash` 在新加坡端点**不支持**，不要添加。
...
**自动化测试**：使用 `test_providers.sh` 一键验证所有 Provider 连通性：
```bash
...
| 9 | 小米 MiMo：`max_tokens` 是否足够（200+）？ | 推理模型 token 不够会返回空内容 |
| 10 | 小米 MiMo：是否误加了 `mimo-v2-flash`？ | 新加坡端点不支持此模型，返回 404 |
...
使用小米 MiMo Omni 处理图片理解任务：
- `/model xm-omni` — 切换到 MiMo V2 Omni（多模态视觉模型）
- 截图报错、分析图表、审查 UI 设计等
...
| 小米模型返回空内容 | max_tokens 设太低（10-50），reasoning 吃掉了全部 token 预算 | 增大 max_tokens 到 200+，所有 MiMo 模型均为推理模型 |
| 小米模型 404 (Not supported) | 误加了 mimo-v2-flash（新加坡端点不支持） | 去掉 mimo-v2-flash，只保留 omni / v2.5-pro / v2.5 / v2-pro |
